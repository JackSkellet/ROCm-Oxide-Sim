...
--scenario <PATH>    optional serde JSON ScenarioConfig
```
...
When `--scenario` is passed, the viewer loads the scenario scene path and uses
the first camera-like sensor mount as the static/reset camera pose. Existing
keyboard controls still move the camera after startup.
